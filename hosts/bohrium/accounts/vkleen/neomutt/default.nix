{ inputs, pkgs, config, nixosConfig, lib, ... }:
let
  account = config.accounts.email.accounts."vkleen";
  yesno = x: if x then "yes" else "no";

  sendMailCommand = pkgs.writeShellScriptBin "sendmail" ''
    tee >(${lib.getExe pkgs.notmuch} insert --folder=sent --create-folder +sent -inbox -unread) | ${nixosConfig.security.wrapperDir}/sendmail -t "$@"
  '';

  new-mail = pkgs.writeShellScriptBin "new-mail" ''
    ${lib.getExe' pkgs.coreutils "tail"} -n+2 | ${lib.getExe pkgs.notmuch} insert
  '';
in
{
  imports = [ ./colors.nix ];
  ...
  home.file."${config.xdg.configHome}/neomutt/${account.name}".text = ''
    ...
    set query_command = "${lib.getExe pkgs.notmuch} address --sort=newest-first --output=sender --output=recipients --deduplicate=address 'from:%s'"
  ...
  home.file.".mailcap".text = ''
    text/html;${lib.getExe pkgs.w3m} -dump -o document_charset=%{charset} '%s'; nametemplate=%s.html; copiousoutput
    image/*;${lib.getExe' pkgs.imv "imv"} %s &>/dev/null &;
    application/pdf;${lib.getExe pkgs.zathura} %s &>/dev/null &;
    image/pdf;${lib.getExe pkgs.zathura} %s &>/dev/null &;
  '';
