...
{
  options = {
    vkleen-tmux.update-environment = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ "WAYLAND_DISPLAY" "HYPRLAND_INSTANCE_SIGNATURE" ];
    };
  };
  config = {
    home.file.".tmux.conf".text = ''
      ...
      bind-key P run "${lib.getExe' pkgs.wl-clipboard "wl-paste"} -n | tmux load-buffer -"
